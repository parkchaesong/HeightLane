...
from models.model.heightlane import *
import wandb
from scipy.interpolate import griddata, splrep, splev
import shutil

model_dir = './heightlane' #model path of verification
''' parameter from config '''
config_file = 'tools/heightlane_config.py'

configs = load_config_module(config_file)
gt_paths = configs.val['gt']
image_paths = configs.val['images']
x_range = configs.x_range
y_range = configs.y_range
meter_per_pixel = configs.meter_per_pixel

'''Post-processing parameters '''
post_conf = -1.7 # Minimum confidence on the segmentation map for clustering
...
model_path = "./ckpt.pth"
tmp_save_path = './results/tmp_openlane' #tmp path for save intermediate result
...
def val():
    ...
    model = load_model(model,
                       model_path)
    print("Use checkpoint: ",model_path)
    model.cuda()
    model.eval()
